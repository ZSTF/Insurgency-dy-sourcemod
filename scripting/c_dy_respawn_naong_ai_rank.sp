...
public Action:Event_PlayerPickSquad_Post( Handle:event, const String:name[], bool:dontBroadcast )
{
	...
	else if (StrContains(g_client_last_classstring[client], "engineer") > -1)
	{
		// Admin medic
		if (GetConVarInt(sm_respawn_enable_donor_tag) == 1 && (GetUserFlagBits(client) & ADMFLAG_ROOT))
			Format(sNewNickname, sizeof(sNewNickname), "%s%s%s[管理][工程] %s", super_medic,super_kill,super_score,g_client_org_nickname[client]);
		// Donor medic
		else if (GetConVarInt(sm_respawn_enable_donor_tag) == 1 && (GetUserFlagBits(client) & ADMFLAG_RESERVATION))
			Format(sNewNickname, sizeof(sNewNickname), "%s%s%s[会员][工程] %s", super_medic,super_kill,super_score,g_client_org_nickname[client]);
		// Normal medic
		else
	...
	else if (StrContains(g_client_last_classstring[client], "spotter") > -1)
	{
		// Admin medic
		if (GetConVarInt(sm_respawn_enable_donor_tag) == 1 && (GetUserFlagBits(client) & ADMFLAG_ROOT))
			Format(sNewNickname, sizeof(sNewNickname), "%s%s%s[管理][神射] %s", super_medic,super_kill,super_score,g_client_org_nickname[client]);
		// Donor medic
		else if (GetConVarInt(sm_respawn_enable_donor_tag) == 1 && (GetUserFlagBits(client) & ADMFLAG_RESERVATION))
			Format(sNewNickname, sizeof(sNewNickname), "%s%s%s[会员][神射] %s", super_medic,super_kill,super_score,g_client_org_nickname[client]);
		// Normal medic
		else
			Format(sNewNickname, sizeof(sNewNickname), "%s%s%s[神射] %s", super_medic,super_kill,super_score,g_client_org_nickname[client]);
	}
	else if (StrContains(g_client_last_classstring[client], "pointman") > -1)
	{
		// Admin medic
		if (GetConVarInt(sm_respawn_enable_donor_tag) == 1 && (GetUserFlagBits(client) & ADMFLAG_ROOT))
			Format(sNewNickname, sizeof(sNewNickname), "%s%s%s[管理][步枪(定点)] %s", super_medic,super_kill,super_score,g_client_org_nickname[client]);
		// Donor medic
		else if (GetConVarInt(sm_respawn_enable_donor_tag) == 1 && (GetUserFlagBits(client) & ADMFLAG_RESERVATION))
			Format(sNewNickname, sizeof(sNewNickname), "%s%s%s[会员][步枪(定点)] %s", super_medic,super_kill,super_score,g_client_org_nickname[client]);
		// Normal medic
		else
			Format(sNewNickname, sizeof(sNewNickname), "%s%s%s[步枪(定点)] %s", super_medic,super_kill,super_score,g_client_org_nickname[client]);
	}
	else if (StrContains(g_client_last_classstring[client], "support") > -1)
	{
		// Admin medic
		if (GetConVarInt(sm_respawn_enable_donor_tag) == 1 && (GetUserFlagBits(client) & ADMFLAG_ROOT))
			Format(sNewNickname, sizeof(sNewNickname), "%s%s%s[管理][机枪] %s", super_medic,super_kill,super_score,g_client_org_nickname[client]);
		// Donor medic
		else if (GetConVarInt(sm_respawn_enable_donor_tag) == 1 && (GetUserFlagBits(client) & ADMFLAG_RESERVATION))
			Format(sNewNickname, sizeof(sNewNickname), "%s%s%s[会员][机枪] %s", super_medic,super_kill,super_score,g_client_org_nickname[client]);
		// Normal medic
		else
			Format(sNewNickname, sizeof(sNewNickname), "%s%s%s[机枪] %s", super_medic,super_kill,super_score,g_client_org_nickname[client]);
	}
	else if (StrContains(g_client_last_classstring[client], "rifleman_at") > -1)
	{
		// Admin medic
		if (GetConVarInt(sm_respawn_enable_donor_tag) == 1 && (GetUserFlagBits(client) & ADMFLAG_ROOT))
			Format(sNewNickname, sizeof(sNewNickname), "%s%s%s[管理][步枪(AT)] %s", super_medic,super_kill,super_score,g_client_org_nickname[client]);
		// Donor medic
		else if (GetConVarInt(sm_respawn_enable_donor_tag) == 1 && (GetUserFlagBits(client) & ADMFLAG_RESERVATION))
			Format(sNewNickname, sizeof(sNewNickname), "%s%s%s[会员][步枪(AT)] %s", super_medic,super_kill,super_score,g_client_org_nickname[client]);
		// Normal medic
		else
			Format(sNewNickname, sizeof(sNewNickname), "%s%s%s[步枪(AT)] %s", super_medic,super_kill,super_score,g_client_org_nickname[client]);
	}
	// Normal class
	else
	{
