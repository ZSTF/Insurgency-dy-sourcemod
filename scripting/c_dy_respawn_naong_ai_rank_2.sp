...
public Action:Event_PlayerPickSquad_Post( Handle:event, const String:name[], bool:dontBroadcast )
{
	...
	decl String:SqlBuffer[256];
	// Get top player or not
	decl String:super_score[32];
	decl String:super_medic[32];
	decl String:super_kill[32];
	int result_num;
	
	Format(SqlBuffer,sizeof(SqlBuffer),"select count(*) from ins_rank where revives > (select revives from ins_rank where steamId=%s)",g_sSteamIdSave[client]);
	DBResultSet resultSet = SQL_Query(g_hDB,SqlBuffer);
	if (SQL_HasResultSet(resultSet))
	{
		SQL_FetchRow(resultSet);
		result_num=SQL_FetchInt(resultSet, 0);
		delete resultSet;
	}
	else
	{
	...
	Format(SqlBuffer,sizeof(SqlBuffer),"select count(*) from ins_rank where kills > (select kills from ins_rank where steamId=%s)",g_sSteamIdSave[client]);
	resultSet = SQL_Query(g_hDB, SqlBuffer);
	if (SQL_HasResultSet(resultSet))
	{
		SQL_FetchRow(resultSet);
		result_num=SQL_FetchInt(resultSet, 0);
	...
	Format(SqlBuffer,sizeof(SqlBuffer),"select count(*) from ins_rank where score > (select score from ins_rank where steamId=%s)",g_sSteamIdSave[client]);
	resultSet = SQL_Query(g_hDB, SqlBuffer);
	if (SQL_HasResultSet(resultSet))
	{
		SQL_FetchRow(resultSet);
	...
	if (StrContains(g_client_last_classstring[client], "medic") > -1)
	{
		...
		if (GetConVarInt(sm_respawn_enable_donor_tag) == 1 && (GetUserFlagBits(client) & ADMFLAG_ROOT))
			Format(sNewNickname,sizeof(sNewNickname),"%s%s%s[管理][医疗] %s",super_medic,super_kill,super_score,g_client_org_nickname[client]);
		// Donor medic
		else if (GetConVarInt(sm_respawn_enable_donor_tag) == 1 && (GetUserFlagBits(client) & ADMFLAG_RESERVATION))
			Format(sNewNickname,sizeof(sNewNickname),"%s%s%s[会员][医疗] %s",super_medic,super_kill,super_score,g_client_org_nickname[client]);
		// Normal medic
		else
			Format(sNewNickname,sizeof(sNewNickname),"%s%s%s[医疗] %s",super_medic,super_kill,super_score,g_client_org_nickname[client]);
	}
	...
	else if (StrContains(g_client_last_classstring[client], "recon_security") > -1)
	{
		// Admin medic
		if (GetConVarInt(sm_respawn_enable_donor_tag) == 1 && (GetUserFlagBits(client) & ADMFLAG_ROOT))
			Format(sNewNickname, sizeof(sNewNickname), "%s%s%s[管理][队长] %s", super_medic,super_kill,super_score,g_client_org_nickname[client]);
		// Donor medic
		else if (GetConVarInt(sm_respawn_enable_donor_tag) == 1 && (GetUserFlagBits(client) & ADMFLAG_RESERVATION))
			Format(sNewNickname, sizeof(sNewNickname), "%s%s%s[会员][队长] %s", super_medic,super_kill,super_score,g_client_org_nickname[client]);
		// Normal medic
		else
			Format(sNewNickname, sizeof(sNewNickname), "%s%s%s[队长] %s", super_medic,super_kill,super_score,g_client_org_nickname[client]);
	}
	else if (StrContains(g_client_last_classstring[client], "sniper") > -1)
	{
		// Admin medic
		if (GetConVarInt(sm_respawn_enable_donor_tag) == 1 && (GetUserFlagBits(client) & ADMFLAG_ROOT))
			Format(sNewNickname, sizeof(sNewNickname), "%s%s%s[管理][狙击] %s", super_medic,super_kill,super_score,g_client_org_nickname[client]);
		// Donor medic
		else if (GetConVarInt(sm_respawn_enable_donor_tag) == 1 && (GetUserFlagBits(client) & ADMFLAG_RESERVATION))
			Format(sNewNickname, sizeof(sNewNickname), "%s%s%s[会员][狙击] %s", super_medic,super_kill,super_score,g_client_org_nickname[client]);
		// Normal medic
		else
			Format(sNewNickname, sizeof(sNewNickname), "%s%s%s[狙击] %s", super_medic,super_kill,super_score,g_client_org_nickname[client]);
	}
	else if (StrContains(g_client_last_classstring[client], "marksman") > -1)
	{
		// Admin medic
		if (GetConVarInt(sm_respawn_enable_donor_tag) == 1 && (GetUserFlagBits(client) & ADMFLAG_ROOT))
			Format(sNewNickname, sizeof(sNewNickname), "%s%s%s[管理][精确] %s", super_medic,super_kill,super_score,g_client_org_nickname[client]);
		// Donor medic
		else if (GetConVarInt(sm_respawn_enable_donor_tag) == 1 && (GetUserFlagBits(client) & ADMFLAG_RESERVATION))
			Format(sNewNickname, sizeof(sNewNickname), "%s%s%s[会员][精确] %s", super_medic,super_kill,super_score,g_client_org_nickname[client]);
		// Normal medic
		else
			Format(sNewNickname, sizeof(sNewNickname), "%s%s%s[精确] %s", super_medic,super_kill,super_score,g_client_org_nickname[client]);
	}
	else if (StrContains(g_client_last_classstring[client], "demolitions") > -1)
	{
		// Admin medic
		if (GetConVarInt(sm_respawn_enable_donor_tag) == 1 && (GetUserFlagBits(client) & ADMFLAG_ROOT))
			Format(sNewNickname, sizeof(sNewNickname), "%s%s%s[管理][爆破] %s", super_medic,super_kill,super_score,g_client_org_nickname[client]);
		// Donor medic
		else if (GetConVarInt(sm_respawn_enable_donor_tag) == 1 && (GetUserFlagBits(client) & ADMFLAG_RESERVATION))
			Format(sNewNickname, sizeof(sNewNickname), "%s%s%s[会员][爆破] %s", super_medic,super_kill,super_score,g_client_org_nickname[client]);
		// Normal medic
		else
			Format(sNewNickname, sizeof(sNewNickname), "%s%s%s[爆破] %s", super_medic,super_kill,super_score,g_client_org_nickname[client]);
	}
	else if (StrContains(g_client_last_classstring[client], "support") > -1)
	{
		// Admin medic
		if (GetConVarInt(sm_respawn_enable_donor_tag) == 1 && (GetUserFlagBits(client) & ADMFLAG_ROOT))
			Format(sNewNickname, sizeof(sNewNickname), "%s%s%s[管理][火力] %s", super_medic,super_kill,super_score,g_client_org_nickname[client]);
		// Donor medic
		else if (GetConVarInt(sm_respawn_enable_donor_tag) == 1 && (GetUserFlagBits(client) & ADMFLAG_RESERVATION))
			Format(sNewNickname, sizeof(sNewNickname), "%s%s%s[会员][火力] %s", super_medic,super_kill,super_score,g_client_org_nickname[client]);
		// Normal medic
		else
			Format(sNewNickname, sizeof(sNewNickname), "%s%s%s[火力] %s", super_medic,super_kill,super_score,g_client_org_nickname[client]);
	}
	else if (StrContains(g_client_last_classstring[client], "rifleman") > -1)
	{
		// Admin medic
		if (GetConVarInt(sm_respawn_enable_donor_tag) == 1 && (GetUserFlagBits(client) & ADMFLAG_ROOT))
			Format(sNewNickname, sizeof(sNewNickname), "%s%s%s[管理][步枪] %s", super_medic,super_kill,super_score,g_client_org_nickname[client]);
		// Donor medic
		else if (GetConVarInt(sm_respawn_enable_donor_tag) == 1 && (GetUserFlagBits(client) & ADMFLAG_RESERVATION))
			Format(sNewNickname, sizeof(sNewNickname), "%s%s%s[会员][步枪] %s", super_medic,super_kill,super_score,g_client_org_nickname[client]);
		// Normal medic
		else
			Format(sNewNickname, sizeof(sNewNickname), "%s%s%s[步枪] %s", super_medic,super_kill,super_score,g_client_org_nickname[client]);
	}
	else if (StrContains(g_client_last_classstring[client], "specialist") > -1)
	{
		// Admin medic
		if (GetConVarInt(sm_respawn_enable_donor_tag) == 1 && (GetUserFlagBits(client) & ADMFLAG_ROOT))
			Format(sNewNickname, sizeof(sNewNickname), "%s%s%s[管理][专家] %s", super_medic,super_kill,super_score,g_client_org_nickname[client]);
		// Donor medic
		else if (GetConVarInt(sm_respawn_enable_donor_tag) == 1 && (GetUserFlagBits(client) & ADMFLAG_RESERVATION))
			Format(sNewNickname, sizeof(sNewNickname), "%s%s%s[会员][专家] %s", super_medic,super_kill,super_score,g_client_org_nickname[client]);
		// Normal medic
		else
			Format(sNewNickname, sizeof(sNewNickname), "%s%s%s[专家] %s", super_medic,super_kill,super_score,g_client_org_nickname[client]);
	}
	// Normal class
	else
	{
		// Admin
		if (GetConVarInt(sm_respawn_enable_donor_tag) == 1 && (GetUserFlagBits(client) & ADMFLAG_ROOT))
			Format(sNewNickname, sizeof(sNewNickname), "%s%s%s[管理] %s", super_medic,super_kill,super_score,g_client_org_nickname[client]);
		// Donor
		else if (GetConVarInt(sm_respawn_enable_donor_tag) == 1 && (GetUserFlagBits(client) & ADMFLAG_RESERVATION))
			Format(sNewNickname, sizeof(sNewNickname), "%s%s%s[会员] %s", super_medic,super_kill,super_score,g_client_org_nickname[client]);
		// Normal player
		else
			Format(sNewNickname, sizeof(sNewNickname), "%s%s%s %s", super_medic,super_kill,super_score,g_client_org_nickname[client]);
	}
				...
				)
				{
					...
					{	
						...
						if (g_playerMedicRevivessAccumulated[iMedic] >= iReviveCap)
						{
							...
							if (iBonus > 1)
								Format(sBuf2, 255,"复活他人:获得了 %i 击杀 和 %i 总分的奖励", iBonus, 10);
							else
				...
				)
				{
					...
					{	
						...
						if (g_playerMedicRevivessAccumulated[iMedic] >= iReviveCap)
						{
							...
							decl String:sBuf2[255];
							if (iBonus > 1)
								Format(sBuf2, 255,"协助复活他人:获得了 %i 击杀 和 %i 总分的奖励", iBonus, 10);
							else
				...
				if (bCanHealPaddle)
				{
					if (iHealth < 100)
					{
						...
						if (g_playerMedicHealsAccumulated[medic] >= iHealthCap && iRewardMedicEnabled == 1)
						{
							...
							decl String:sBuf2[255];
							 if (iBonus > 1)
							 	Format(sBuf2, 255,"治疗他人:获得了 %i 击杀的奖励,治疗HP量: %i.", iBonus, iHealthCap);
							 else
 ...
 */
 ...
 public GetMyRank(client){
 	...
 	if (g_hDB != INVALID_HANDLE)
 	{
 		...
 		{
 			PrintToChat(client,"%c等待系统读取数据库", GREEN);
 		}
 	}
 	else
 	{
 		PrintToChat(client, "%c排名系统当前维护中", GREEN);
 	}
 }
 ...
 public SQLJoinMsgGetPlayerCount(Handle:owner, Handle:hndl, const String:error[], any:client)
 {
 	...
 	if(SQL_FetchRow(hndl))
 	{
 		...
 		PrintToChatAll("\x04%N\x01 加入了战斗. \x05(排名: %i/总玩家数: %i)", client, g_iRank[client], iPlayerCount);
 ...
 public SQLGetMyRank(Handle:owner, Handle:hndl, const String:error[], any:client){
 	...
 	if(SQL_FetchRow(hndl)) 
 	{
 		...
 		PrintToChat(client,"%c总分: %i | 击杀: %i | 复活: %i | 治疗: %i | 死亡: %i | 爆头: %i | 自杀: %i", GREEN, RAscore, RAkills, RArevives, RAheals, RAdeaths, RAheadshots, RAsucsides);
 	} else {
 		PrintToChat(client, "%c你的排名还未读取完成!", GREEN);
 	}
 }
 ...
 public SQLShowRankToChat(Handle:owner, Handle:hndl, const String:error[], any:client){
 	...
 	if(SQL_FetchRow(hndl)) 
 	{
 		...
 		// Display rank
 		PrintToChat(client,"%c当前排名: %i (of %i).", GREEN, g_iRank[client], iPlayerCount);
 	} else {
 		PrintToChat(client, "%c你的排名还未读取完成!", GREEN);
 	}
 }
 ...
 public SQLTopShow(Handle:owner, Handle:hndl, const String:error[], any:client){
 	...
 	if (SQL_HasResultSet(hndl))
 	{
 		...
 		while (SQL_FetchRow(hndl))
 		{
 			...
 			Format(text,127," - 总分: %i | 击杀: %i | 死亡: %i", score, kills, deaths);
 			DrawPanelText(hPanel, text);
 		}
 	} else {
